...
$s_total        = "SELECT COUNT(*) AS Clientes FROM clientes WHERE status<>'D'";
$q_total        = mysqli_query($conexao,$s_total);
$total          = mysqli_fetch_assoc($q_total);
$totalRegistros = $total['Clientes'];

///// NÚMERO TOTAL DE REGISTRO COM FILTRAGEM
$s_total_busca        = "SELECT COUNT(*) AS ClientesBusca FROM clientes WHERE status<>'D' ".$searchQuery;
$q_total_busca        = mysqli_query($conexao,$s_total_busca);
$total_busca          = mysqli_fetch_assoc($q_total_busca);
$totalRegistrosFiltro = $total_busca['ClientesBusca'];

///// LISTAR
$s_listar = "SELECT * FROM clientes WHERE status<>'D' ".$searchQuery." ORDER BY ".$columnName." ".$columnSortOrder." LIMIT ".$row.",".$rowperpage;
...
while($row = mysqli_fetch_assoc($q_listar)) {
  ...
  $data[] = array( 
    'id'         => encrypt_decrypt('encrypt', $row['id_cliente']),
    'id_cliente' => $row['id_cliente'],
    'nome'       => $row['nome'],
    'status'     => '<a href="javascript:void(0)" class="statusRegistro btn btn-sm btn-'.$status_class.'" data-id="'.encrypt_decrypt('encrypt', $row['id_cliente']).'" data-acao="clienteStatus" id="btStatus-'.encrypt_decrypt('encrypt', $row['id_cliente']).'">'.$status.'</a>',
    'funcoes'    => '<div class="dropdown">
          ...
          <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
            <li><a class="dropdown-item" href="./clientes-alterar/'.encrypt_decrypt('encrypt', $row['id_cliente']).'"><i class="fa-solid fa-pen-to-square"></i> Alterar</a></li>
            <li><a class="excluirRegistro dropdown-item text-danger" href="javascript:void(0);" data-id="'.encrypt_decrypt('encrypt', $row['id_cliente']).'" data-acao="clientesApagar"><i class="fa-solid fa-trash-can"></i> Apagar</a></li>
          </ul>
        </div>'
   );
}
