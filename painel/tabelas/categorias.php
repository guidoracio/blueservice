...
$s_total        = "SELECT COUNT(*) AS Categorias FROM categorias WHERE status<>'D'";
$q_total        = mysqli_query($conexao,$s_total);
$total          = mysqli_fetch_assoc($q_total);
$totalRegistros = $total['Categorias'];

///// NÚMERO TOTAL DE REGISTRO COM FILTRAGEM
$s_total_busca        = "SELECT COUNT(*) AS CategoriasBusca FROM categorias WHERE status<>'D' ".$searchQuery;
$q_total_busca        = mysqli_query($conexao,$s_total_busca);
$total_busca          = mysqli_fetch_assoc($q_total_busca);
$totalRegistrosFiltro = $total_busca['CategoriasBusca'];

///// LISTAR
$s_listar = "SELECT * FROM categorias WHERE status<>'D' ".$searchQuery." ORDER BY ".$columnName." ".$columnSortOrder." LIMIT ".$row.",".$rowperpage;
...
while($row = mysqli_fetch_assoc($q_listar)) {
  ...
  $data[] = array( 
    'id'           => encrypt_decrypt('encrypt', $row['id_categoria']),
    'id_categoria' => $row['id_categoria'],
    'nome'         => $row['nome'],
    'status'       => '<a href="javascript:void(0)" class="statusRegistro btn btn-sm btn-'.$status_class.'" data-id="'.encrypt_decrypt('encrypt', $row['id_categoria']).'" data-acao="categoriaStatus" id="btStatus-'.encrypt_decrypt('encrypt', $row['id_categoria']).'">'.$status.'</a>',
    'funcoes'      => '<div class="dropdown">
          ...
          <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
            <li><a class="dropdown-item" href="./categorias-alterar/'.encrypt_decrypt('encrypt', $row['id_categoria']).'"><i class="fa-solid fa-pen-to-square"></i> Alterar</a></li>
            <li><a class="excluirRegistro dropdown-item text-danger" href="javascript:void(0);" data-id="'.encrypt_decrypt('encrypt', $row['id_categoria']).'" data-acao="categoriasApagar"><i class="fa-solid fa-trash-can"></i> Apagar</a></li>
          </ul>
        </div>'
   );
}
